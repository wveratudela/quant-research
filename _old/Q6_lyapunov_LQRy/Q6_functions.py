...
def classify_regime(vix, trend):
    # Four-state regime
    if vix < 15 and trend == 1:
        return 'Bull'
    elif vix < 20 and trend == 1:
        return 'Neutral'
    elif vix < 30 or trend == 0:
        return 'Bear'
    else:
        return 'Crisis'
...
def min_variance_constrained(target_return, mu, cov, max_weight=0.25):
    n = len(mu)
    constraints = [
        {'type': 'eq', 'fun': lambda w: np.sum(w) - 1},
        {'type': 'eq', 'fun': lambda w: portfolio_return(w, mu) - target_return}
